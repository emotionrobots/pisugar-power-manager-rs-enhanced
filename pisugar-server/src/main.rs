...
///------------------------------------------------------------------
///  Handling stream, including:
///    - Handle requests
///    - Send button events to sink
///    - Handle response from sink 
///------------------------------------------------------------------
    ...
    T: 'static + AsyncRead + AsyncWrite + Send,
{
    ...
    let (tx, rx) = unbounded();   // Create in-memory stream with unbounded capacity

    // Handle request and respond to sink
    ...
    // Forward button event to sink
    tokio::spawn(event_rx.map(|event| Ok(Some(event))).forward(tx));

    // Catch what the sink sends back
    tokio::spawn(
        rx.map(|s| match s {
            Some(s) => {
...
async fn main() -> std::io::Result<()> {
    ...
    // Create core from config or default; continue until creation succeeds
    let core;
    loop {
    ...
    // Create an Event Watch which returns a Sender channel "event_tx" 
    // and a Receiver "event_rx".  Only the last sent value is retained.
    ...
    loop {
        ...
        // Poll button press and return result in event_tx
